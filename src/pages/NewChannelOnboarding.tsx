...
const NewChannelOnboarding = () => {
  ...
  const handleAdminProfileImageUpload = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const files = Array.from(event.target.files || []);
    if (files.length > 0) {
          ...
          if (uploadChannelImgResponse.status === 200) {
            ...
            if (channelDataResponse.status === 200) {
              if (adminImageFile !== null) {
                ...
                const uploadAdminImgResponse = await axiosInstance.post(
                  `/image/upload`,
                  adminImgFormData,
                  {
                    headers: {
                ...
                if (uploadAdminImgResponse.status === 200) {
                  ...
                  adminFormData.append(
                    "channelId",
                    channelDataResponse.data.data.id
                  );
                  adminFormData.append(
                    "profileImageUrl",
                    uploadAdminImgResponse.data.data.url
                  );
              ...
              >
                <div
                  style={{
                    ...
                    backgroundImage:
                      channelProfileImage !== undefined
                        ? `url(${channelProfileImage})`
                        : `url(${defaultChannelProfile})`,
                  }}
                ></div>
              </div>
              <div style={{ height: "40px" }}></div>
