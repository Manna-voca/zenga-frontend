...
const MeetupHome = () => {
    ...
    const getMeetupInfo = async () => {
        if(loading) return;
        try{
            const uri = `${SERVER_URL}/party/list?channelId=${localStorage.getItem("channelId")}` +
                        (cursorMeetupId ? `&partyId=${cursorMeetupId}` : "")+"&size=15";
            ...
            await axios.get(`${uri}`, CONFIG).then((res) => {
                ...
                if(res.data.data.hasNext === false){
                    setHasNext(false);
                    setCursorMeetupId(Number(res.data.data.content[res.data.data.content.length-1].partyId));
                }
                else{
                    setCursorMeetupId(Number(res.data.data.content[res.data.data.content.length-1].partyId));
                }
                        ...
                        {meetupList.length === 0 ? (
                            ...
                            <>
                                ...
                                <div
                                    ...
                                    style={{ display: 'flex',
                                            flexDirection: 'column', height: 'calc(100vh - 125px)',
                                            maxHeight: 'calc(100vh - 125px)', overflowY: 'scroll',
